...
/*
	if($AndroidApp == true){
		if ($_SERVER['REQUEST_URI'] == '/') {header("Location: https://andiroid.info/toolbox");}
	}
*/
?>
...
<div class="bit-60">
	...
	<div style="padding:0px 0px;">
		...
		<?php
			...
			if($currentFileName == ''){
				$currentFileName = 'startpage';
			} elseif($currentFileName == 'index'){
				$currentFileName = 'startpage';
			}
			
			if($currentFileName == 'startpage'){
				$projectCoreSQL="SELECT * FROM p_core WHERE p_key='startpage' LIMIT 1";
			} else {
				$projectCoreSQL="SELECT * FROM p_core WHERE p_slug='$currentFileName' LIMIT 1";
			}
			...
			if($user['group'] == 2) {
				...
				<a class="admineditbutton" href="/cms/content/project/edit?id='.$p_id.'&amp;key='.$p_key.'"><span style="">&#9998;</span></a>
				';
				if($p_key != 'startpage'){
					echo '<div id="'.$p_id.'-trash" onclick="trashProject(this)"><img style="height:25px;width:25px;" src="/img/core/trash.svg" alt=""></div>';
				}
			...
			if($num_rows == 0){
				...
				if($currentFileName == 'archive'){
					...
					echo '
						...
						<h5 style="width:100%;text-align:center;padding-bottom:30px">uploaded - '.$p_created.'</h5>
