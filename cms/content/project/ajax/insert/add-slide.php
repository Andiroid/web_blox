...
$fileName = $_FILES['fileUPLOAD']['name'][0];
		...
		$target_dir = $_SERVER['DOCUMENT_ROOT'] ."/img/project/slide/origin/";
		...
		if (move_uploaded_file($_FILES["fileUPLOAD"]["tmp_name"][0], $target_file)) {
			...
			$sql = "INSERT INTO item_slide (slide_nr, slide_file)
			VALUES ('$p_id', '$out')";
			...
			if ($conn->query($sql) === TRUE) {
				...
				$input_path = $_SERVER['DOCUMENT_ROOT'] .'/img/project/slide/origin/';
				$output_path = $_SERVER['DOCUMENT_ROOT'] .'/img/project/slide';
				$input_name = $out;
				$output_name = $out;
				$x = 0;

				if (strtoupper($imageFileType) == 'SVG'){
					$file = $_SERVER['DOCUMENT_ROOT'] .'/img/project/slide/origin/'.$out;
